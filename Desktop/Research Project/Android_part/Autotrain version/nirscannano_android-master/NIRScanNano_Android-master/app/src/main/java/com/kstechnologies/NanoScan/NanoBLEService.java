    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                KSTNanoSDK.mBluetoothGatt.requestConnectionPriority(1);//CONNECTION_PRIORITY_HIGH
                /*
                new Thread(new Runnable() {
                    @Override
                    public void run() {
                        while (!mtuConfirmed) {
                            KSTNanoSDK.mBluetoothGatt.requestMtu(100);
                            mtuRequestCounter++;
                            try {
                                Thread.sleep(1000);
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt,
            ...
            // When the first time scan button was clicked, it will request and receive many status information of characteristic
            // The following actions will just make change of scandata for characteristic
            if (times == 0) {
                //Log.d(TAG, "first scan");
                // Writing scandata for characteristic
                if(characteristic == KSTNanoSDK.NanoGattCharacteristic.mBleGattCharGSDISRetSerialScanDataStruct) {
                    ...
                    times++; //increase the times of clicking scan button
                } else {
                ...
                // The following scans instead of the first time.
                //----------------------------------------------------------------------------------------------------
            } else {
                //Log.d(TAG, "other scans");
                // Writing scandata for characteristic
                if(characteristic == KSTNanoSDK.NanoGattCharacteristic.mBleGattCharGSDISRetSerialScanDataStruct) {
                    final byte[] data = characteristic.getValue();
                    final StringBuilder stringBuilder = new StringBuilder(data.length);
                    ...
                    if (debug)
                        Log.d("__SIZE", "new ScanData size:" + scanData.size());
                    times++; //increase the times of clicking scan button
                } else if (characteristic == KSTNanoSDK.NanoGattCharacteristic.mBleGattCharGSDISStartScanNotify) {
                    final byte[] data = characteristic.getValue();
                    if (data[0] == (byte) 0xff) {
                        if (debug)
                            Log.d(TAG, "Scan data is ready to be read");
    ...
    public void onCreate() {
        ...
        mStartScanReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                ...
                Log.d("test", "BLE mStartScanReceiver 1182");
